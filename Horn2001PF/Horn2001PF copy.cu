...
int main (void){
    ...
    Items items ; 
    input_items(items,"./input/initial") ; items.PFthick = 0.01 ;
        ...
        for(j=0;j<items.ny;j++){
            for(l=0;l<items.nz;l++){
                ...
                phi[phi.size()-1] = 0.5 - 0.5*tanh(
                    (posz[posz.size()-1] - (h2 + (posx[posx.size()-1]-3.0)*slope_eta0 )  )/items.PFthick*2
                ) ; // 6m
                ...
                tau.push_back( 1.0*3.0*pow(10,-6)/pow(items.c,2)/items.dt+0.5) ;
    ...
    float dt_ratio = 0.25 ; int dt_change = 3 ;
    ...
    // for(int timestep=1;timestep<1+2;timestep++){
        if(timestep==1){
            ...
            reset_pressure<float><<<numBlocks, blockSize>>>(d_items, d_pressure, d_f, dt_ratio) ;
        ...
        if(timestep==dt_change*items.save_interval+1){
            ...
            reset_pressure<float><<<numBlocks, blockSize>>>(d_items, d_pressure, d_f, 1.0/dt_ratio) ;
            cudaMemcpy(d_items, item.data(), item.size() * sizeof(float), cudaMemcpyHostToDevice) ;
        } // */
