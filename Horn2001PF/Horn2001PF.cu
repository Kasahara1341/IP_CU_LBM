...
int main (void){
    ...
    ////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////
    items.save_interval = 2/items.dt ; items.total_count= 400/items.dt ;
    ////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////
            ...
            for(j=0;j<items.ny;j++){
                for(int divj=0;divj<divy[j];divj++){
                    for(l=0;l<items.nz;l++){
                        float x = ( i + (2*divi+1)/(2*divx[i]) )*items.dx*items.ratiox ;
                        float y = ( j + (2*divj+1)/(2*divy[j]) )*items.dx*items.ratioy ;
    ...
    float rhoL=1000, rhoH=1020.0, muL=1.*pow(10,-5), muH=muL ;//1.016*pow(10,-3) ;
    for(i=0;i<items.nx;i++){
        for(int divi=0;divi<divx[i];divi++){
            for(j=0;j<items.ny;j++){
                for(int divj=0;divj<divy[j];divj++){
                    for(l=0;l<items.nz;l++){
                        if(lnum[i*items.ny*items.nz+j*items.nz+l]<0){continue ;}
                        ...
                        phi[phi.size()-1] = 0.5 - 0.5*tanh( (local_z -(h2+(local_x-3)*slope_eta0 ))/items.PFthick*2.0 ) ; // 
                        ...
                        for(k=0;k<items.num_velocity;k++){
                            ...
                            g.push_back(items.weight[k]*phi[sal.size()-1]*(1 + 3*tmp + 4.5*tmp*tmp - 1.5*(pow(vel_x[vel_x.size()-1],2)+pow(vel_y[vel_x.size()-1],2)+pow(vel_z[vel_x.size()-1],2))/pow(items.c,2) )) ;
                        }
                    }
                }
            }
    ...
    // for(int timestep=1;timestep<1+2;timestep++){
        ...
        Force         <float> <<<numBlocks, blockSize>>>(d_items, Boussi_flag, d_neib, d_f, d_feq, d_tau, d_Fk, d_Fx, d_Fy, d_Fz, d_pressure, d_rho, d_sal, d_phi, d_posx, d_posy, d_posz) ;
