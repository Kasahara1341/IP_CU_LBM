    ...
    with open(f"./data/Fakhari_LBM{counter:04d}.csv", "r", encoding="utf-8", errors="", newline="") as csv_file:
        ...
        for row in f:
            if row[0]=="num_calc":
                ...
                pressure [count] = float(row[3])
                sal [count]      = float(row[4])
                velx[count]      = float(row[5])
                vely[count]      = float(row[6])
                velz[count]      = float(row[7])
                Fx  [count]      = float(row[8])
                Fy  [count]      = float(row[9])
                Fz  [count]      = float(row[10])
                count+=1
    # 以下はVTK形式のデータ作成
    points = [] ; cells = [] ; cell_types = [] ; 
    sal_data = [] ; pressure_data = [] ; vel_data = [] ; force_data = []
    point_lookup = {}
    point_id = 0
    for i in range(count):
        p0 = (posx[i] - dx/2.0, posy[i] - dy/2.0, posz[i] - dz/2.0)
        p0 = (posx[i] - dx/2.0, posy[i] - dy/2.0, posz[i] - dz/2.0)
        p1 = (posx[i] + dx/2.0, posy[i] - dy/2.0, posz[i] - dz/2.0)
        p2 = (posx[i] + dx/2.0, posy[i] + dy/2.0, posz[i] - dz/2.0)
        p3 = (posx[i] - dx/2.0, posy[i] + dy/2.0, posz[i] - dz/2.0)
        p4 = (posx[i] - dx/2.0, posy[i] - dy/2.0, posz[i] + dz/2.0)
        p5 = (posx[i] + dx/2.0, posy[i] - dy/2.0, posz[i] + dz/2.0)
        p6 = (posx[i] + dx/2.0, posy[i] + dy/2.0, posz[i] + dz/2.0)
        p7 = (posx[i] - dx/2.0, posy[i] + dy/2.0, posz[i] + dz/2.0)
