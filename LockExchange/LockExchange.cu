...
int main (void){
    ...
    ////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////
    items.save_interval = 1.0/items.dt ; items.total_count= 30/items.dt ;
    items.save_interval = items.total_count/30 ;
    // items.total_count=1 ; items.save_interval=1 ;
    ////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////
            ...
            for(j=0;j<items.ny;j++){
                for(int divj=0;divj<divy[j];divj++){
                    for(l=0;l<items.nz;l++){
                        ...
                        float local_y = (j + (2*divj+1.0)/(2.0*divy[j]) )*items.dx*items.ratioy ;
                        posx.push_back(local_x) ; posz.push_back(local_z) ;
                        posy.push_back( local_y ) ;
                        delX.push_back( items.dx*items.ratiox/divx[i]) ; delY.push_back( items.dx*items.ratioy/divy[j]) ;
                        phi.push_back(1) ;           
                        if(local_x<0.4){sal.push_back(1.22);} else{sal.push_back(0);}
    ...
    item.push_back(0) ; item.push_back(items.dx) ;
    ...
    // for(int timestep=1;timestep<1+2;timestep++){
        ...
        if(timestep%items.save_interval==0){
            ...
            if(isnan(vel_x[items.ny*items.nz])!=0){
