...
int main (void){
    ...
    float H0, c0, d0, Delta, radius = 0.204 ; 
    // d0 = radius/0.6 ; H0 = 0.311*d0 ; // 4. B case 1
    d0 = radius/0.7 ; H0 = 0.514*d0 ; // 4. B case 2
    c0 = sqrt(9.81*(d0+H0)) ; Delta = d0*sqrt(4*d0/(3*H0)) ;
    ...
    items.PFthick = 5.5*items.dx ; items.sigma = 0.072 *1;
    ...
    ////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////
    items.total_count= 2.5/items.dt ; items.save_interval = items.total_count/25 ;
    // items.total_count= 1.0/items.dt ; items.save_interval = items.total_count/10 ;

    // float tmp=0.3 ; 
    // items.save_interval*=tmp ; items.total_count*=tmp ;
    ////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////
            ...
            for(j=0;j<items.ny;j++){
                for(int divj=0;divj<divy[j];divj++){
                    for(l=0;l<items.nz;l++){
                        float x = ( i + (2.0*divi+1)/(2.0*divx[i]) )*items.dx*items.ratiox ;
                        float y = ( j + (2.0*divj+1)/(2.0*divy[j]) )*items.dx*items.ratioy ;
                        float z=(0.5+l)*items.dx ;
                        if(pow(x-5,2)+pow(z,2)>pow(radius,2)){
                            lnum.push_back(items.num_calc) ;
                            items.num_calc+=1 ;
                        }
                        else{lnum.push_back(-1) ; 
                            // printf("i=%d j=%d k=%d divx=%d\n",i,j,l,divx[i]) ;                            
                        }
                    }
                }
            }
            ...
            for(j=0;j<items.ny;j++){
                for(int divj=0;divj<divy[j];divj++){
                    for(l=0;l<items.nz;l++){
                        ...
                        float local_x = (i + (2.0*divi+1.0)/(2.0*divx[i]) )*items.dx*items.ratiox ;
                        ...
                        for(k=0;k<items.num_velocity;k++){
                            Fk.push_back(0) ; f.push_back(0) ;g.push_back(0) ;
                        }
                        int_tmp+= 1 ;
                    }
                }
    ...
    hydrostatic_pressure(items,Boussi_flag,neib,pressure,rho,f,posz, float(d0/2.0)) ;
    // hydrostatic_pressure(items,Boussi_flag,neib,pressure,rho,f,posz, float(items.dx*items.nz/2.0)) ;
    ...
    // for(int timestep=1;timestep<1+2;timestep++){
        ...
        // velocity field
        wall_function <float> <<<numBlocks, blockSize>>>(d_items, d_delX, d_delY, 1, 0, 0, IDX_wall1_size, d_wall1, d_v, d_w, d_u, d_Fy, d_Fz, d_rho) ;
        wall_function <float> <<<numBlocks, blockSize>>>(d_items, d_delX, d_delY, 1, 0, 0, IDX_wall3_size, d_wall3, d_v, d_w, d_u, d_Fy, d_Fz, d_rho) ;
        wall_function <float> <<<numBlocks, blockSize>>>(d_items, d_delX, d_delY, 0, 0, 1, IDX_wall5_size, d_wall5, d_u, d_v, d_w, d_Fx, d_Fy, d_rho) ;
        wall_function <float> <<<numBlocks, blockSize>>>(d_items, d_delX, d_delY, 0, 0, 1, IDX_wall6_size, d_wall6, d_u, d_v, d_w, d_Fx, d_Fy, d_rho) ;
        wall_function <float> <<<numBlocks, blockSize>>>(d_items, d_delX, d_delY, 0, 1, 0, IDX_wall2_size, d_wall2, d_u, d_w, d_v, d_Fx, d_Fz, d_rho) ;
        wall_function <float> <<<numBlocks, blockSize>>>(d_items, d_delX, d_delY, 0, 1, 0, IDX_wall4_size, d_wall4, d_u, d_w, d_v, d_Fx, d_Fz, d_rho) ; // */
        ...
        if(timestep%items.save_interval==0){
            ...
            for(k=0;k<items.num_velocity;k++){
                ...
                // cout << "i="<<i <<" "<<neib[i*items.num_velocity]<< " neib[3]="<<neib[i*items.num_velocity+3]<<endl;
