    ...
    for count2 in range(nxyz):
        if(x[count2]>=pointx and x[count2]<=pointx+dx*12 ):
            if(salinity[count2]>salt and salinity[count2+1]<salt):
                hei.append((z[count2]+np.abs(salt-salinity[count2])/(np.abs(salt-salinity[count2+1])+np.abs(salt-salinity[count2]))*dx-0.2914)/0.2914)
                break
        else:
            continue
...
for count1 in range(start,end): 
    ...
    for row in f:   #行ごとに回す
        ...
        if row[0]!="num_calc" and row[0]!="x       ":
            ...
            tmp+=1 
    csv_file.close()
    get_hei(len(salinity),5.3224,dx,0.5,salinity,x,z,hei1)
    get_hei(len(salinity),5.8724,dx,0.5,salinity,x,z,hei2)
    get_hei(len(salinity),6.3224,dx,0.5,salinity,x,z,hei3)
    ti.append(0.1*count1*np.sqrt(9.81/0.2914) - 3.7)
