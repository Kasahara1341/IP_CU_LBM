...
def process_file(counter):
    ...
    for row in f:
        if row[0]=="num_calc":
            num_calc=0
            phi = np.zeros(num_calc) ; posx=np.zeros(num_calc) ; posy=np.zeros(num_calc) ; posz=np.zeros(num_calc)
    ...
    phi_avg = np.full((len(z_unique), len(x_unique)),np.nan)
    for i, x in enumerate(x_unique):
        for j, z in enumerate(z_unique):
            # (x,z)が一致するインデックスを取得
            mask = (posx == x) & (posz == z)
            if np.any(mask):
                phi_avg[j, i] = np.mean(phi[mask])  # y方向の平均
    X, Z = np.meshgrid(x_unique, z_unique)

    plt.figure(figsize=(8,1))
    plt.pcolormesh(X, Z, phi_avg, shading='auto', cmap='jet',vmin=0,vmax=1.0)
    # plt.tricontourf(posx,posz,phi,levels=100,cmap="jet",vmin=0,vmax=24.26)
    plt.xlabel("time step %04d"%counter)
    # plt.colorbar()
    plt.savefig("figure/phi_%04d.png"%counter,bbox_inches='tight', dpi=300)
...
def main():
    ...
    #make gif and mp4
    files = sorted(glob.glob('./figure/phi*.png'))  
    titlegif="./phi"  
