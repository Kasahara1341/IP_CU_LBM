...
for count1 in range(start,end): 
    ...
    f = csv.reader(csv_file, delimiter=",", doublequote=True, lineterminator="\r\n", quotechar='"', skipinitialspace=True)
    tmp=0 
    salinity  =np.zeros(nx*ny*nz) ; x = np.zeros(nx*ny*nz) ; y=np.zeros(nx*ny*nz) ; z=np.zeros(nx*ny*nz) 
    for row in f:   #行ごとに回す
        if row[0]=='':  #空白が来たら(値が出きったら)ループから抜ける
            break
        if row[0]!="num_calc" and row[0]!="x       ":
            salinity[tmp]=row[4] ; x[tmp]=row[0] ; y[tmp]=row[1] ; z[tmp]=row[2]
            tmp+=1 
    csv_file.close()
    for count2 in range(nx*ny*nz):
        if(x[count2]>=1.47 and x[count2]<=1.47+dx*ratio ): # y=0
        # if(x[count2]>=1.47 and x[count2]<=1.47+dx*ratio and y[count2]==0.135): # center
            if(salinity[count2]>salt and salinity[count2+type]<salt):
                heiB.append((z[count2]+np.abs(salt-salinity[count2])/(np.abs(salt-salinity[count2+type])+np.abs(salt-salinity[count2]))*dx/type-0.087)*100)
                break
        else:
            continue
    ti.append(2*count1)
...
np.savetxt("./make_result/1_%d_A.csv"%ratio,data, delimiter=",")
