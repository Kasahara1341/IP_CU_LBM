...
int main (void){
    ...
    ////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////
    items.save_interval = 1.0/items.dt ; items.total_count= 10.0/items.dt ;
    ...
    IB.push_back(massB[0]*(pow(a_axis,2) + pow(b_axis,2) )/8.0) ; 
    ...
    for(k=0;k<items.num_IBMpoints;k++){
        ...
        oposw.push_back(a_axis*sin(theta1)) ; // y
        if(k!=0){ellipce_length+=sqrt(pow(oposw[k*3+0]-oposw[k*3-3],2)+pow(oposw[k*3+1]-oposw[k*3-2],2)) ;}
        oposw.push_back(0); 

        // 楕円の法線ベクトルを算出　原点を0とする楕円の法線ベクトル成分は(2x/a^2 , 2y/b^2)
        onB_vec.push_back(2.0*oposw[k*3+0]) ;
        onB_vec.push_back(2.0*oposw[k*3+1]) ; // y
        onB_vec.push_back(0) ;
        ...
        for(i=0;i<3;i++){ 
            ...
            nB_vec.push_back(nbvec/norm_nBvec) ; posw.push_back(posB[i]+pos) ;
        }
        float dist1 = 100 ;
        for(i=0;i<items.num_calc;i++){
    ...
    for(int timestep=1 ; timestep<items.total_count+1 ; timestep++){
        ...
        if(timestep%items.save_interval==0){
            ...
            cout<<"Q0= "<<quaternion[0]<<"  Q2= "<< quaternion[3]<<" thetax "<<acos(quaternion[0])*180/3.141592 *2<<" thetaz "<<asin(quaternion[3])*180/3.141592 *2<<   endl;
        }
        resetF<float><<<numBlocks, blockSize>>>(d_items, d_Fx, d_Fy, d_Fz, Fx.size()) ;
