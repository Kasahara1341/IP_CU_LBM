...
__device__ void  Lagrange_interpolation(Typ *coeff, Typ *posl_x, Typ x_alpha, int num_point){
    ...
    for(i=0;i<num_point;i++){
        ...
        for(j=i+1;j<num_point;j++){coeff[i] *= (x_alpha - posl_x[j])/(posl_x[i]-posl_x[j]) ;}
    }
}
...
__global__ void set_out(Typ *items, int *neib, Typ *ftmp, Typ *fout, Typ *fin, Typ *finy, Typ *posx, Typ *posy, Typ *delX, Typ *delY){
    ...
    if(id_rho<items[IDX_num_calc]){
        int k, cp[27]={0}, cm[27]={0} ; 
        int cip[27]={0}, cim[27]={0} ; set_cpm(cp,cm,cip,cim,(int)items[IDX_Q]) ;
        ...
        for(k=0;k<num1;k++){ // monolinear x direction
            ...
            Lagrange_interpolation<Typ>(coeff,posl_x,x_alpha,5) ;
        ...
        for(k=0;k<num3;k++){  // bilinear ? 7,8,9,10 , 19~26
            Typ flaxx=0, flaxy=0, flaxxy=0 ;
            int idf[25] ; idf[12]=id_f ; 
            idf[13] = neib[id_f    + cxp[k_number3[k]]]*(int)items[IDX_Q] ; idf[14] = neib[idf[13] + cxp[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[11] = neib[id_f    + cxm[k_number3[k]]]*(int)items[IDX_Q] ; idf[10] = neib[idf[11] + cxm[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[17] = neib[id_f    + cyp[k_number3[k]]]*(int)items[IDX_Q] ; idf[22] = neib[idf[17] + cyp[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[ 7] = neib[id_f    + cym[k_number3[k]]]*(int)items[IDX_Q] ; idf[ 2] = neib[idf[ 7] + cym[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[18] = neib[id_f    + cp [k_number3[k]]]*(int)items[IDX_Q] ; idf[24] = neib[idf[18] + cp [k_number3[k]]]*(int)items[IDX_Q] ;
            idf[ 6] = neib[id_f    + cm [k_number3[k]]]*(int)items[IDX_Q] ; idf[ 0] = neib[idf[ 6] + cm [k_number3[k]]]*(int)items[IDX_Q] ;
            idf[ 8] = neib[id_f    + cip[k_number3[k]]]*(int)items[IDX_Q] ; idf[ 4] = neib[idf[ 8] + cip[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[16] = neib[id_f    + cim[k_number3[k]]]*(int)items[IDX_Q] ; idf[20] = neib[idf[16] + cim[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[19] = neib[idf[14] + cyp[k_number3[k]]]*(int)items[IDX_Q] ; idf[ 9] = neib[idf[14] + cym[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[15] = neib[idf[10] + cyp[k_number3[k]]]*(int)items[IDX_Q] ; idf[ 5] = neib[idf[10] + cym[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[23] = neib[idf[22] + cxp[k_number3[k]]]*(int)items[IDX_Q] ; idf[21] = neib[idf[22] + cxm[k_number3[k]]]*(int)items[IDX_Q] ;
            idf[ 3] = neib[idf[ 2] + cxp[k_number3[k]]]*(int)items[IDX_Q] ; idf[ 1] = neib[idf[ 2] + cxm[k_number3[k]]]*(int)items[IDX_Q] ;

            Typ coeffx[5], coeffy[5], posl_x[5], x_alpha ;
            x_alpha = posx[id_rho] + items[IDX_cx(k_number3[k])]/items[IDX_c]*(delX[id_rho]-items[IDX_dz])/2.0 ;
            posl_x[0]=posx[idf[10]/int(items[IDX_Q])] ; posl_x[1]=posx[idf[11]/int(items[IDX_Q])] ; posl_x[2]=posx[id_rho] ; 
            posl_x[3]=posx[idf[13]/int(items[IDX_Q])] ; posl_x[4]=posx[idf[14]/int(items[IDX_Q])] ;
            set_posl_x<Typ>(items,posl_x,k_number3[k]) ; // 計算点周りの座標決定が完了
            Lagrange_interpolation<Typ>(coeffx,posl_x,x_alpha,5) ;
            for(int i=0;i<5;i++){
                flaxx += coeffx[i]*ftmp[idf[i+10]+k_number3[k]] ; // 10~14
            }

            x_alpha = posy[id_rho] + items[IDX_cy(k_number3[k])]/items[IDX_c]*(delY[id_rho]-items[IDX_dz])/2.0 ;
            posl_x[0]=posy[idf[ 2]/int(items[IDX_Q])] ; posl_x[1]=posy[idf[ 7]/int(items[IDX_Q])] ; posl_x[2]=posy[id_rho] ; 
            posl_x[3]=posy[idf[17]/int(items[IDX_Q])] ; posl_x[4]=posy[idf[22]/int(items[IDX_Q])] ;
            set_posl_y<Typ>(items,posl_x,k_number3[k]) ; // 計算点周りの座標決定が完了
            Lagrange_interpolation<Typ>(coeffy,posl_x,x_alpha,5) ;
            for(int i=0;i<5;i++){
                flaxy += coeffy[i]*ftmp[idf[2+i*5]+k_number3[k]] ; // 2,7,12,17,22
            }

            for(int i=0;i<5;i++){
                for(int j=0;j<5;j++){
                    flaxxy += coeffx[i]*coeffy[j]*ftmp[idf[i+j*5]+k_number3[k]] ; // 2,7,12,17,22
                }
            }
            ...
            atomicAdd(&fin[id_f+k_number3[k]], flaxxy*(items[IDX_dz]*items[IDX_dz])/(delX[idf[13]/int(items[IDX_Q])]*delY[idf[17]/int(items[IDX_Q])])) ;
            atomicAdd(&fin[neib[id_f   +cxm[k_number3[k]]]*(int)items[IDX_Q]+k_number3[k]], (flaxy-flaxxy*items[IDX_dz]/delX[idf[17]/int(items[IDX_Q])])*items[IDX_dz]/delY[idf[17]/int(items[IDX_Q])] );//* Typ(1-int(items[IDX_dz]/delX[id_rho])) ) ;
            atomicAdd(&finy[neib[id_f  +cym[k_number3[k]]]*(int)items[IDX_Q]+k_number3[k]], (flaxx-flaxxy*items[IDX_dz]/delY[idf[13]/int(items[IDX_Q])])*items[IDX_dz]/delX[idf[13]/int(items[IDX_Q])] );//* Typ(1-int(items[IDX_dz]/delY[id_rho])) ) ; // */
        } 
    }
}
...
__global__ void set_tmp(Typ *items, int *neib, Typ *f, Typ *ftmp, Typ *fout, Typ *fin){
    ...
    int id_f = id_rho*(int)items[IDX_Q] ;
    if(id_rho<items[IDX_num_calc]){
        int k, cp[27]={0}, cm[27]={0}, cip[27]={0}, cim[27]={0} ; set_cpm(cp,cm,cip,cim,(int)items[IDX_Q]) ;
        for(k=0;k<items[IDX_Q];k++){
...
template __device__ void set_posl_x<float>(float*, float*, int) ;
template __device__ void set_posl_x<double>(double*, double*, int) ;
template __device__ void Lagrange_interpolation<float>(float*,float*,float,int) ;
template __device__ void Lagrange_interpolation<double>(double*, double*,double,int) ;
